...
export default function MobileUploadQR({ session, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-6" onClick={onClose}>
      ...
      >
        <button onClick={onClose} className="absolute top-3 right-3 text-[color:var(--muted)] hover:text-[color:var(--ink)]" aria-label={t("common.close")}>
          <X size={18} />
        </button>
        <Smartphone size={22} className="mx-auto mb-4 text-[color:var(--muted)]" />
        <h3 className="font-serif-display text-2xl tracking-tight mb-2">{t("mobileQr.title")}</h3>
        <p className="text-sm text-[color:var(--ink)]/70 mb-6">
          {t("mobileQr.instructions")}
        </p>
        {session ? (
          ...
          <p className="text-sm text-red-600">{t("mobileQr.error")}</p>
        )}
        <p className="text-xs text-[color:var(--muted)] mt-6">{t("mobileQr.expiry")}</p>
      </div>
    </div>
  );
}
