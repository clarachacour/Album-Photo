...
export default function ContactPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.detail || t("contact.errorToast"));
    } finally {
      setSending(false);
    }
  };
      ...
      <div className="max-w-[700px] mx-auto">
        ...
        {sent ? (
          <div className="border border-[color:var(--border-soft)] p-10 text-center">
            <p className="font-serif-display text-2xl tracking-tight mb-2">{t("contact.thanks")}</p>
            <p className="text-sm text-[color:var(--muted)]">{t("contact.willReply")}</p>
          </div>
        ) : (
          <form onSubmit={submit}>
            <div className="grid md:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="eyebrow block mb-2">{t("contact.name")}</label>
                <input className={inputClass} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
              </div>
              <div>
                <label className="eyebrow block mb-2">{t("contact.email")}</label>
              ...
              <div className="md:col-span-2">
                <label className="eyebrow block mb-2">{t("contact.message")}</label>
                <textarea
                  rows={6}
